Feature: rake db:migrate
  ...
  Scenario: db:migrate should work with octopus
    When I run inside my Rails project "rake db:migrate" with environment "development"
  ...
  Scenario: db:migrate:redo should work with octopus
    When I run inside my Rails project "rake db:migrate VERSION=20100720172715" with environment "development"
    When I run inside my Rails project "rake db:migrate VERSION=20100720172730" with environment "development"
    When I run inside my Rails project "rake db:migrate:redo" with environment "development"
  ...
  Scenario: after running rake db:migrate
    When I run inside my Rails project "rake db:abort_if_pending_migrations" with environment "development"
    Then the output should contain "pending migrations"
    When I run inside my Rails project "rake db:migrate" with environment "development"
    When I run inside my Rails project "rake db:abort_if_pending_migrations RAILS_ENV=development" with environment "development"
